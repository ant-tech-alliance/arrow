...
class ARROW_EXPORT PlasmaClient {
 public:
  ...
  Status CreateQueue(const ObjectID& object_id, int64_t data_size, std::shared_ptr<Buffer>* data,
                     int device_num=0, bool evict_without_client = false, bool recreate_queue = false);
  ...
  Status PushQueueItem(const ObjectID& object_id, uint8_t* data, uint32_t data_size, uint64_t timestamp = -1);

  // TODO(j): The reader will notify plasma store to evict each queue item if notify_each_item is true, we will notify the store
  // each block when it's false. We can send a fake item to deal with the last block issue
  // It will increase the amount of notification(1000x) when notify_each_item is true, we should del this flag later
  ...
  Status CreateQueueItem(const ObjectID& object_id, uint32_t data_size, std::shared_ptr<Buffer>* data,
                         uint64_t& seq_id, bool&is_first_item, uint64_t timestamp=-1);

  Status CreateQueueItem(const ObjectID& object_id, uint32_t data_size, std::shared_ptr<Buffer>* data, uint64_t& seq_id);

  Status SealQueueItem(const ObjectID& object_id, uint64_t seq_id, std::shared_ptr<Buffer> data, bool is_first_item);
 ...
 private:
  ...
  // Invoked by GetQueueItem, read from the fd which was generated by socketpair in function SubscribeQueue
  Status GetQueueNotification(int fd, uint64_t* seq_id, uint64_t* data_offset,
